...
import { saveFormAsDraft, submitTransactionForm } from './clerkFormSubmission';
...
export const useClerkTransactionFormActions = () => {
  ...
  // Save form as draft using Clerk
  const saveForm = useCallback(async () => {
    try {
      await saveFormAsDraft(state, getToken, userId);
  ...
  const submitForm = useCallback(async () => {
    ...
    try {
      await submitTransactionForm(state, getToken, userId);
  ...
  }, [state, resetForm, validateCurrentStep, getToken, userId]);
  ...
  const calculateCommission = useCallback((): CommissionBreakdown => {
    console.log('DEBUG - calculateCommission called with formData:', state.formData);

    // Use default values if missing to prevent NaN calculations
    const transactionValue = state.formData.transactionValue || 100000;
    const commissionRate = state.formData.commissionRate || 5;
    const { coBroking, agentTier = 'Advisor' } = state.formData;

    console.log('DEBUG - Using values:', { transactionValue, commissionRate, agentTier });

    // Calculate basic values with minimum value
    const totalCommission = Math.max(0.01, (transactionValue * commissionRate) / 100);
    console.log('DEBUG - Calculated totalCommission:', totalCommission);

    // Update the form data with the calculated commission amount
    try {
      if (totalCommission > 0) {
        console.log('DEBUG - Updating commissionAmount to:', totalCommission);
        dispatch({
          type: 'UPDATE_FORM_DATA',
          payload: { commissionAmount: totalCommission }
        });
      }
    } catch (error) {
      console.error('DEBUG - Error updating commission amount:', error);
    }
