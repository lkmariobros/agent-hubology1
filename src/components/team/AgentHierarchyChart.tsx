...
const AgentNode: React.FC<AgentNodeProps> = ({ 
  ...
  return (
    ...
    >
      <div 
        className="flex flex-col items-center p-3 bg-card rounded-lg border shadow-sm hover:shadow-md transition-shadow cursor-pointer"
      ...
      >
        ...
        {isAdmin && (
          <div className="mt-1 text-xs text-muted-foreground">
            {agent.downline?.length || 0} members
          </div>
        )}
      </div>
...
const AgentHierarchyChart: React.FC<AgentHierarchyChartProps> = ({ 
  data, 
  onAgentClick,
  isAdmin = false 
}) => {
